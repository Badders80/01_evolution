"""
Firebase Authentication Middleware for KYC API
...
def require_auth(f):
    ...
    Also accepts: X-Firebase-Token header (from proxy pattern)
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not id_token:
            return jsonify({"error": "Missing Firebase token. Expected: X-Firebase-Token or Authorization: Bearer <token>"}), 401
